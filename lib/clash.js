...
export class Clash {
    ...
    log = { path: config.get("clash-log"), text: "" }
    constructor(options) {
        let args = [];
        const { d, extCtl, extUi, f, secret, t, v, daemon, dryrun, clashCMD, clashLog } = options;
        if (clashCMD) {
        ...
        if (daemon === "pm2") {
            ...
            cmds.push(`nohup ${this.clashCMD} ${args.join(" ")} >  ${this.log.path} &`)
        }
        else if (daemon === "&") {
            cmds.push(`${this.clashCMD} ${args.join(" ")} > ${this.log.path} &`)
        } else {
