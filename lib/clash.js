...
export class Clash {
    ...
    constructor(options) {
        let args = [];
        const { d, extCtl, extUi, f, secret, t, v, daemon, dryrun } = options;
        if (d) {
            args.push(`-d "${d}"`)
        }
        if (f) {
        ...
        if (daemon === "pm2") {
            execSync(`pm2 start clash --name clash -- ${args.join(" ")}`, { "encoding": "utf-8" })
        }
        else if (daemon === "screen") {
            try {
                execSync(`screen -X -S clash quit`, { "encoding": "utf-8" })
            } catch (e) { logger.info(0, e) }
            execSync(`screen -S clash -dm clash ${args.join(" ")}`, { "encoding": "utf-8" })
        }
        else if (daemon === "nohup&") {
            this._cp = exec(`nohup clash ${args.join(" ")} > $HOME/clash.out &`, { "encoding": "utf-8" })
        }
        else if (daemon === "&") {
            this._cp = exec(`clash ${args.join(" ")} > $HOME/clash.out &`, { "encoding": "utf-8" })
        }

        const command = `clash ${args.join(" ")}`
