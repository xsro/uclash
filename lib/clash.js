...
import { projectFolder, asCachePath } from "./util.js";

export class Clash {
    execOptions = { cwd: projectFolder };
    clashCMD = "clash";
    processName = "uclash"
    secret = false;
    time = {
    ...
    log = { path: asCachePath("clash.log"), text: "" }
    constructor(options) {
        let args = [];
        ...
        if (daemon === "pm2") {
            execSync(`pm2 start ${this.clashCMD} --name ${this.processName} -- ${args.join(" ")}`, { "encoding": "utf-8" })
        }
        else if (daemon === "screen") {
            try {
                execSync(`screen -X -S ${this.processName} quit`, { "encoding": "utf-8" })
            } catch (e) { logger.info(0, e) }
            execSync(`screen -S ${this.processName} -dm ${this.clashCMD} ${args.join(" ")}`, { "encoding": "utf-8" })
        }
        else if (daemon === "nohup&") {
            this._cp = exec(`nohup ${this.clashCMD} ${args.join(" ")} > $HOME/clash.out &`, { "encoding": "utf-8" })
        }
        else if (daemon === "&") {
            this._cp = exec(`${this.clashCMD} ${args.join(" ")} > $HOME/clash.out &`, { "encoding": "utf-8" })
        }

        const command = `${this.clashCMD} ${args.join(" ")}`
        logger.info(4, daemon + ": " + command);
        ...
        if (!dryrun && !daemon) {
            function daemonExec(child, command, execOptions) {
                ...
                const clashLog = options.clashLog ? options.clashLog.trim() : "redirect";
                if (clashLog === "redirect")
                    command += `> tmp/clash.log`
                ...
                if (clashLog === "all" || clashLog === "force") {
                    const level = clashLog === "force" ? 7 : 0
                    child.stdout.on("data", data => logger.info(level, data));
                }
            }
