...
const folderProperty = [
    "ui-folder", "config-folder", "cache-folder", "tmp-folder"
...
export const paths = {
    ...
    tmp: function (str) {
        const tempdir = config.get("tmp-folder")
        return path.resolve(tempdir, str)
    }
}
...
function parseConfig(_config) {
    ...
    for (const key in config) {
        config[key] = resolveVariable(config[key]);
        if (folderProperty.includes(key)) {
            !existsSync(config[key]) && mkdirSync(config[key], { recursive: true })
        }
    }
    return config
}
...
export const execOpts = { cwd: paths.projectFolder, stdio: "inherit", encoding: "utf-8" };
