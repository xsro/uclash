...
export default async function updateClashProfile(config, outsideProfileDst, useGit, push) {
    ...
    if (outsideProfileDst) {
        copyProfileDestinations.push(outsideProfileDst)
    }
    ...
    for (const copyDst of copyProfileDestinations) {
        if (existsSync(copyDst)) {
            const configText = readFileSync(copyDst, { encoding: "utf-8" });
            if (configText !== cc.text) {
                writeFileSync(copyDst, cc.text, { encoding: "utf-8" });
                logger.info(3, "copied to" + copyDst);
            }
        } else {
            console.warn("file not found " + copyDst)
        }
    }
