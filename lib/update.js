...
import { asAbsolutePath, projectFolder } from "./util.js";
import { logger } from "./logger.js";
import parse from "./parser/parser.js";

export default async function updateClashProfile(config, profileDst, outsideProfileDst, useGit, push) {

    const former = existsSync(profileDst) ? readFileSync(profileDst, { encoding: "utf-8" }) : "";
    ...
    if (!changed) {
        ...
        writeFileSync(profileDst, cc.text, { encoding: "utf-8" });
...
runs on ${process.platform}-${process.arch}`;
    ...
    if (changed && useGit) {
        execSync("git pull", { cwd: projectFolder });
        execSync("git add clash/", { cwd: projectFolder });
        try {
            execSync(`git commit -F "tmp/1.txt"`, { cwd: projectFolder });
            if (push) {
                const out = execSync(`git push`, { encoding: "utf-8", cwd: projectFolder });
