 ...
 */
async function parse(config, allowCache = false) {
    let parsed = null;
    if (config.parser.type === 1) {
        parsed = await parser1.parse(config, allowCache)
    }
    if (config.parser.type === 2) {
        parsed = await parser2.parse(config, allowCache)
    }
    if (parsed) {
        parsed.updateTimeMsg = flat(parsed.updateTime)
        parsed.text = `#${new Date()} with uclash ${pack.version}
