...
export async function parse(profile, allowCache) {
    ...
    while (true) {
        ...
        if (idx > -1) {
            ...
            if (item.updateTime) {
                ...
                for (const url of item.use) {
                    const { owner, repo, branch, path } = githublink(url)
    ...
    for (const i in profile["proxy-groups"]) {
        ...
        if (group.use) {
            const text = await download2(group.use, allowCache);
            const obj = YAML.parse(text);
    ...
    return { text: YAML.stringify(clashProfile), updateTime }
}