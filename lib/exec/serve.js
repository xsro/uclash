...
function getMime(file) {
    const k = Object.keys(mime).find(a => "." + a === path.extname(file))
    return k ? mime[k] : mime.txt
}

URL.prototype.absLink = function () {
    const url = this.toString();
    const idx = url.indexOf(this.pathname)
    return url.substring(idx)
}

export async function serve(opt = { port: 7899, profile: undefined }) {
    let { port, profile } = opt;
    if (port === undefined) port = 7899;
    const cprofile = profile
        ? await getClashConfig(profile)
        : undefined
    const server = http.createServer(async function (req, res) {
        ...
        const reqUrl = req.originalUrl
            ...
            : new URL("http://" + ip + ":" + req.socket.localPort + req.url)
        console.log(new Date().toLocaleString() + " request " + reqUrl)

        let folder = config.get("serve-folder");
        let hooksFolder = config.get("serve-hook-folder");
        if (process.env["DEBUG_serve_example"]) {
            folder = paths.resources("serve-example")
            hooksFolder = paths.resources("serve-hook-example")
        }
                    ...
                    try {
                        const r = await caller.call({
                            ...
                            context: {
                                ...
                                genPAC, download, download2, cprofile
                            },
                            prevent: function () {
                                prevent = true
                            },
